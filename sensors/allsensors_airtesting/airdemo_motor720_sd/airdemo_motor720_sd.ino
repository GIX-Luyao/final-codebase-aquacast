...
static const unsigned long FULL_720_TIME = 2700;  // 720° 對應時間
...
void setup() {
  ...
  if (SD.begin(SD_CS, sdSPI)) {
    ...
    Serial.print("SD Ready. CSV File: ");
    ...
    for (int i = 0; i < sampleCount; i++) {
      ...
      if (!triggered[i]) {
        ...
        if (millis() - seqStartMs >= targetDepths[i] * SAMPLE_INTERVAL_MS) {
          ...
          Serial.print("\n>>> Sample ");
          Serial.print(i + 1);
          Serial.print(" Triggered (Time Based at ");
          Serial.print(targetDepths[i]);
          Serial.println("m)");
